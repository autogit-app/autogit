...
class CreateCodeRepoScreen extends ConsumerStatefulWidget {
  const CreateCodeRepoScreen({super.key});

  @override
  ConsumerState<CreateCodeRepoScreen> createState() =>
      _CreateCodeRepoScreenState();
}

class _CreateCodeRepoScreenState extends ConsumerState<CreateCodeRepoScreen> {
  List<EnvironmentTemplate>? _templates;
  String? _error;
  bool _loading = true;
  ...
  Future<void> _load() async {
    ...
    try {
      final list = await fetchEnvironments(token: token);
      if (mounted) {
        setState(() {
          _templates = list;
          _loading = false;
  ...
  Future<void> _createFromTemplate(EnvironmentTemplate template) async {
    ...
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        title: Text('Create code repo: ${template.name}'),
        content: TextField(
    ...
    try {
      await createCodeRepoFromTemplate(
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Repository $username/$repoName created from template'),
        ),
      );
      context.push(
    ...
    if (!hasToken) {
      return Scaffold(
        body: CustomScrollView(
          slivers: [
            const LargeAppBar(title: 'Create code repo'),
            SliverFillRemaining(
              hasScrollBody: false,
              child: Center(
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        'Sign in with GitHub to create code repos from templates',
        ...
        slivers: [
          ...
          SliverToBoxAdapter(
            child: Padding(
              ...
              child: Text(
                'Choose an environment template from autogit-app/environments. Your new repo will be created from it.',
